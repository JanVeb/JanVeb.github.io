...
import { getInstrument, loadInstruments } from './load-instrument';
...
class ReactMidiPlayer extends React.Component {
  ...
  async componentDidMount() {
    ...
    const instruments = await loadInstruments(this.midiPlayer);
    this.setState({
      areInstrumentsLoaded: true,
    ...
    this.midiPlayer.on('midiEvent', (midiEvent) => {
      ...
      const { instrumentName, noteName, key, channel } = payload;
...
//https://github.com/JanVeb/Piano-Scores/blob/main/demo_midi_chopin.mid
const App = () => (
  <ReactMidiPlayerDemo url={`assets/midiFiles/demo_midi_chopin.mid`} />
);
